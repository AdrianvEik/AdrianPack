...
def test_compress_width():
    ...
    test.assert_array_almost_equal(compress_width(arr, 5),
    ...
    test.assert_array_almost_equal(compress_width(arr, 0.2),
                                   np.array([0.86/3, 2.54/4, 2.66/3], dtype=np.float32),
                                   decimal=7)
    print("Test 2 Passed in %s ns" % (time.time_ns() - t))
    t = time.time_ns()
    test.assert_array_almost_equal(compress_width(np.flip(arr), 0.2),
    ...
    test.assert_array_almost_equal(compress_width(arr, 0.0007),
